...
bool jvalue::read_plist(const string& strdoc, string* pstrerr , bool* is_binary )
{
	return read_plist(strdoc.data(), strdoc.size(), pstrerr, is_binary);
}

bool jvalue::read_plist(const char* pdoc, size_t len , string* pstrerr , bool* is_binary )
{
	if (NULL == pdoc) {
		return false;
	}
...
bool jreader::_read_object(jvalue& jval)
{
	...
	while (_read_token(token_name)) {
		if (jtoken::E_JTOKEN_OBJECT_END == token_name.type) {
			return true;
		}
		...
		if (!_read_value(jval[name.c_str()])) {
			return false;
		}
...
bool jpreader::_read_dictionary(jvalue& pval)
{
	...
	while (_read_token(key)) {
		if (ptoken::E_PTOKEN_DICTIONARY_END == key.type) {
			return true;
		}
...
bool jpreader::_read_array(jvalue& pval)
{
	...
	while (true) {
		...
		if (ptoken::E_PTOKEN_ARRAY_END == elem.type) {
			return true;
		} else if (ptoken::E_PTOKEN_KEY == elem.type) {
			string strkey = "";
