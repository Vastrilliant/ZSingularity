...
uint32_t ZArchO::ReallocCodeSignSpace(const string& strNewFile)
{
	...
	uint32_t uNewLength = m_uCodeLength + ZUtil::ByteAlign(((m_uCodeLength / 4096) + 1) * (20 + 32), 4096) + 16384;
...
bool ZArchO::InjectDylib(bool bWeakInject, const char* szDylibFile)
{
	...
	if (m_uLoadCommandsFreeSpace > 0 && m_uLoadCommandsFreeSpace < uDylibCommandSize) {
